...
func TestCommandInstaller_InstallCommand(t *testing.T) {
	...
	err := installer.InstallCommand("simple-skill", "")
	if err != nil {
		t.Fatalf("安装空 Command 应该成功（跳过）: %v", err)
	}

	// 测试安装不存在的 Command（应该返回错误）
	err = installer.InstallCommand("non-existent-plugin", "test-command")
	if err == nil {
...
func TestCommandInstaller_UninstallCommand(t *testing.T) {
	...
	err := installer.UninstallCommand("non-existent-command")
	if err != nil {
		t.Errorf("卸载不存在的 Command 不应该报错: %v", err)
	}

	// 测试卸载空 Command ID（应该成功，不报错）
	err = installer.UninstallCommand("")
	if err != nil {
		t.Errorf("卸载空 Command ID 不应该报错: %v", err)
	}

	// 创建测试文件
	commandsDir := filepath.Join(tempDir, ".cursor", "commands")
	...
	// 卸载 Command
	if err := installer.UninstallCommand("test-command"); err != nil {
