...
function activate(context) {
  console.log("CoCursor Efficiency System \u5DF2\u6FC0\u6D3B");
  statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
  statusBarItem.text = "$(loading~spin) CoCursor \u542F\u52A8\u4E2D...";
  statusBarItem.show();
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("cocursor.sidebar", sidebarProvider)
  );
}
function deactivate() {
  if (backendProcess) {
  ...
  console.log("CoCursor Efficiency System \u5DF2\u505C\u7528");
}
async function startBackendServer(context) {
  var _a, _b;
  const config = vscode.workspace.getConfiguration("cocursor");
  const port = config.get("serverPort", 8080);
  try {
    await axios_default.get(`http://localhost:${port}/health`);
    statusBarItem.text = "$(check) CoCursor \u5DF2\u8FDE\u63A5";
    statusBarItem.tooltip = `\u540E\u7AEF\u670D\u52A1\u8FD0\u884C\u5728\u7AEF\u53E3 ${port}`;
    return;
  } catch (error) {
  }
  const backendPath = path.join(context.extensionPath, "backend", "bin", "server");
  const backendPathWin = backendPath + ".exe";
  try {
    const execPath = process.platform === "win32" ? backendPathWin : backendPath;
    backendProcess = child_process.spawn(execPath, [], {
    ...
    backendProcess.on("close", (code) => {
      ...
      statusBarItem.text = "$(error) CoCursor \u5DF2\u65AD\u5F00";
    });
    await waitForServer(port);
    statusBarItem.text = "$(check) CoCursor \u5DF2\u8FDE\u63A5";
    statusBarItem.tooltip = `\u540E\u7AEF\u670D\u52A1\u8FD0\u884C\u5728\u7AEF\u53E3 ${port}`;
  } catch (error) {
    statusBarItem.text = "$(warning) CoCursor \u542F\u52A8\u5931\u8D25";
  ...
  _getHtmlForWebview(webview) {
    return `<!DOCTYPE html>
        ...
        <body>
            ...
            <button class="button" onclick="openDashboard()">\u6253\u5F00\u5B8C\u6574\u9762\u677F</button>
            <button class="button" onclick="addTask()">\u6DFB\u52A0\u4EFB\u52A1</button>
            <h4>\u4ECA\u65E5\u4EFB\u52A1</h4>
            <ul class="task-list">
                <li class="task-item">\u{1F4DD} \u793A\u4F8B\u4EFB\u52A1 1</li>
                <li class="task-item">\u2705 \u793A\u4F8B\u4EFB\u52A1 2\uFF08\u5DF2\u5B8C\u6210\uFF09</li>
            </ul>
            <script>
